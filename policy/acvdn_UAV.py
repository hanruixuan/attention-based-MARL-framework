...
from network.acvdn_net import ACVDNNet
...
class ACVDN:
    def __init__(self, args):
        ...
        self.eval_vdn_net = ACVDNNet(input_shape, args)  # 把agentsQ值加起来的网络
        self.target_vdn_net = ACVDNNet(input_shape, args)
        self.args = args
        if self.args.cuda:
            self.eval_mlp.cuda()
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_total_eval = self.eval_vdn_net(q_evals, batch['s'], batch['o'], batch['u'])
        q_total_target = self.target_vdn_net(q_targets, batch['s_next'], batch['o_next'], next_actions)
    ...
    def _get_inputs(self, batch, transition_idx):
        ...
        len_obs = obs.shape[2]
        input_new = obs.reshape(-1, len_obs)
        inputs_next_new = obs_next.reshape(-1, len_obs)
        # inputs, inputs_next = [], []
        # inputs.append(obs)
        ...
        return input_new, inputs_next_new
